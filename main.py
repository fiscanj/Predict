...
# Classe pour le filtre de Kalman (léger et rapide)
class KalmanFilter:
...
# Télécharger des données historiques pour la simulation
tickers = ["NVDA", "META"]
...
prices = bars.pivot(columns="symbol", values="close").dropna()
returns = prices.pct_change().dropna()

# Paramètres HFT
window_size = 20  # Fenêtre courte pour réactivité
capital = 100
max_leverage = 2
coint_window = 60  # Co-intégration sur 60 minutes
vol_window = 20   # Volatilité et corrélation sur 20 minutes
portfolio = [capital]
stop_loss = -1.0  # Perte max par trade (réduit pour HFT)
min_trade_size = 0.01  # Quantité minimale tradable
...
# Simulation HFT
actual_returns = []
beta = 1.0  # Coefficient initial de co-intégration
...
for t in range(window_size, len(prices) - 1):
    ...
    # Volatilité et corrélation dynamique
    ...
    # Co-intégration (toutes les 60 minutes)
    if t >= coint_window and (t - window_size) % coint_window == 0:
        ...
        if coint_result[1] < 0.05:  # Co-intégrés
            beta = np.polyfit(train_prices['NVDA'].tail(coint_window), train_prices['META'].tail(coint_window), 1)[0]
    
    # Seuils dynamiques
    error_threshold_nvda = 0.05 * (1 + vol_nvda)  # Réduit pour HFT
    error_threshold_meta = 0.05 * (1 + vol_meta)
    arb_threshold = 0.8 * (1 + vol_avg)  # Plus agressif
    
    # Kalman pour estimation rapide
    ...
    # Arbitrage avec co-intégration
    spread = train_prices['NVDA'] - beta * train_prices['META']
    ...
    corr_signal = 1 if corr_dynamic < 0.4 else -1 if corr_dynamic > 0.9 else 0  # Plus strict pour HFT
    ...
    # Levier dynamique
    leverage = max_leverage if vol_avg < 0.015 else 1.0  # Plus strict pour HFT
    ...
    # PnL avec stop-loss
    actual_return = returns.iloc[t + 1].mean()
